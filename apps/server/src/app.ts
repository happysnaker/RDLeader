...
class InvalidCommandInputError extends Error {
...
function assertCliToken(value: unknown, fieldName: string): string {
  if (typeof value !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u.test(value)) {
    throw new InvalidCommandInputError(`${fieldName} must be a safe CLI token`);
  }
  return value;
}

function assertBoundedText(value: unknown, fieldName: string, maxLength: number): string {
  if (typeof value !== 'string') {
    throw new InvalidCommandInputError(`${fieldName} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed || trimmed.length > maxLength || /[\u0000]/u.test(trimmed)) {
    throw new InvalidCommandInputError(`${fieldName} is invalid`);
  }
  return trimmed;
}

function assertIsoDateTime(value: unknown, fieldName: string): string {
  const text = assertBoundedText(value, fieldName, 80);
  if (Number.isNaN(Date.parse(text))) {
    throw new InvalidCommandInputError(`${fieldName} must be a valid datetime`);
  }
  return text;
}

function assertJsonFields(value: unknown): string {
  const text = assertBoundedText(value, 'fields', 4000);
  try {
    const parsed = JSON.parse(text) as unknown;
    if (!Array.isArray(parsed) && (typeof parsed !== 'object' || parsed === null)) {
      throw new InvalidCommandInputError('fields must be a JSON object or array');
    }
  } catch (error) {
    if (error instanceof InvalidCommandInputError) throw error;
    throw new InvalidCommandInputError('fields must be valid JSON');
  }
  return text;
}

function commandInputError(reply: { code: (statusCode: number) => { send: (payload: unknown) => unknown } }, error: unknown) {
  if (error instanceof InvalidCommandInputError) {
    return reply.code(400).send({ message: 'invalid command input' });
  }
  throw error;
}

function buildMeegoWorkitemUpdateArgs(input: {
  workItemId: string;
  projectKey: string;
  fields: string;
}) {
  return [
    ...
    'update',
    '--project-key',
    assertCliToken(input.projectKey, 'projectKey'),
    '--work-item-id',
    assertCliToken(input.workItemId, 'workItemId'),
    '--fields',
    assertJsonFields(input.fields),
  ];
}

function buildMeegoCommentCreateArgs(input: {
  workItemId: string;
  projectKey: string;
  ...
  return [
    ...
    assertCliToken(input.workItemId, 'workItemId'),
    '--comment-content',
    assertBoundedText(input.commentContent, 'commentContent', 2000),
  ];
}

function buildTechReviewMeetingArgs(input: {
  summary: string;
  description: string;
  start: string;
  end: string;
  ...
  if (!Array.isArray(input.attendeeIds) || input.attendeeIds.length === 0 || input.attendeeIds.length > 30) {
    throw new InvalidCommandInputError('attendeeIds must be a non-empty bounded array');
  }
  ...
  return [
    ...
    'user',
    '--summary',
    assertBoundedText(input.summary, 'summary', 200),
    '--description',
    assertBoundedText(input.description, 'description', 2000),
    '--start',
    assertIsoDateTime(input.start, 'start'),
    '--end',
    assertIsoDateTime(input.end, 'end'),
    '--attendee-ids',
    input.attendeeIds.map((id) => assertCliToken(id, 'attendeeId')).join(','),
    '--json',
  ];
}
...
function buildMeegoWorkitemUpdateCommand(input: {
  ...
  return ['bytedcli', ...buildMeegoWorkitemUpdateArgs(input)];
}
...
async function updateMeegoWorkitem(input: {
  ...
  const { stdout } = await execFileAsync('bytedcli', buildMeegoWorkitemUpdateArgs(input));
  try {
    return JSON.parse(stdout);
  } catch {
    return { ok: true, raw: stdout };
...
function buildMeegoCommentCreateCommand(input: {
  ...
  return ['bytedcli', ...buildMeegoCommentCreateArgs(input)];
}
...
async function createMeegoComment(input: {
  ...
  const { stdout } = await execFileAsync('bytedcli', buildMeegoCommentCreateArgs(input));
  try {
    return JSON.parse(stdout);
  } catch {
    return { ok: true, raw: stdout };
...
function buildTechReviewMeetingCommand(input: {
  ...
  return ['lark-cli', ...buildTechReviewMeetingArgs(input)];
}
...
async function createTechReviewMeeting(input: {
  ...
  const { stdout } = await execFileAsync('lark-cli', buildTechReviewMeetingArgs(input));
  try {
    return JSON.parse(stdout);
  } catch {
    return { ok: true, raw: stdout };
...
export async function buildApp(options: {
  ...
  app.post('/employees/:employeeId/actions/meego-workitem-update', async (request, reply) => {
    ...
    let command: string[];
    try {
      command = buildMeegoWorkitemUpdateCommand(body);
    } catch (error) {
      return commandInputError(reply, error);
    }
  ...
  app.post('/employees/:employeeId/actions/meego-comment-create', async (request, reply) => {
    ...
    let command: string[];
    try {
      command = buildMeegoCommentCreateCommand(body);
    } catch (error) {
      return commandInputError(reply, error);
    }
  ...
  app.post('/employees/:employeeId/actions/schedule-tech-review', async (request, reply) => {
    ...
    let command: string[];
    try {
      command = buildTechReviewMeetingCommand(body);
    } catch (error) {
      return commandInputError(reply, error);
    }
